...
const sendReservationEmail = async ({ to, replyTo, subject, text, html }) => {
...
app.get('/api/health', (_req, res) => {
  res.json({ ok: true, service: 'lau-lash-atelier' });
});
...
app.post('/api/reservations', async (req, res) => {
  ...
  if (!process.env.SMTP_USER || !process.env.SMTP_PASS || !process.env.RECIPIENT_EMAIL) {
    return res.status(500).json({
      message: 'A Gmail beállítások még nincsenek konfigurálva. Ellenőrizd a .env fájlt.',
    });
  }
  ...
  try {
    ...
    return res.json({ message: 'Köszönjük! A foglalási kérésedet elküldtük.' });
  } catch (error) {
...
app.listen(PORT, () => {
  console.log(`Lau Lash Atelier backend running on http://localhost:${PORT}`);
});
